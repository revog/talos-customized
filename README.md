...
| Image | Purpose |
|-------|---------|
| `ghcr.io/revog/talos-ufs-installer:<version>` | Installer for machine config |
...
4. **System Extensions** *(this fork)*: `build.yml` accepts a `system_extensions` input and bakes the listed extension images into both the generated ISO and the installer image — see [System Extensions](#system-extensions)
...
To build a local installer image with extensions instead (step 7 variant, once imager is pushed):
...
. **Monitor upstream releases** (`check-release.yml`): Daily check for new stable Talos releases; passes the `SYSTEM_EXTENSIONS` repository variable through to `build.yml`
